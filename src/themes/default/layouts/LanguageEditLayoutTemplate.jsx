...
import {BaasicFormControls, BaasicTable, LocalizationEditTable, Loadable, BaasicButton} from "core/components";
        ...
        <ListLayout loading={loaderStore.loading}>
            <PageFooter>
                <div>
                    ...
                    {routeBack && <BaasicButton className="btn btn--med btn--primary display-ib" label={t("EDIT_FORM_LAYOUT.CANCEL")} onClick={routeBack} />}
                </div>
            </PageFooter>

            <form onSubmit={form.onSubmit} className="clearfix">
                <LocalizationEditTable form={form} loaderStore={loaderStore} item={item} />
            </form>
        </ListLayout>
    );
}
