...
function DashboardTemplate({ dashboardViewStore, t, rootStore }) {
	...
	const LineChartContainer = () => (
		<Chart style={{ height: 260 }}>
			...
			<ChartSeries>
				<ChartSeriesItem color="#bc6d11" name={`Total granted: ${dataContributions[dataContributions.length - 1] ? `${currencyFormat(dataContributions[dataContributions.length - 1])}` : '$' + (0).toFixed(2).toString()}`} type="line" data={dataContributions} />
				<ChartSeriesItem color="#223a5e" name={`Total contributed: ${dataGrants[dataGrants.length - 1] ? `${currencyFormat(dataGrants[dataGrants.length - 1])}` : '$' + (0).toFixed(2).toString()}`} type="line" data={dataGrants} />
			</ChartSeries>
		</Chart>
	);
	return (
