        ...
        window.addEventListener('unhandledrejection', (event) => {
            ...
            if (typeof reason === 'object') {
                if (reason.request !== undefined && reason.statusCode !== undefined) {
                    ...
                    if (self.rootStore.routerStore.routerState.routerName !== 'master.error') {
...
function getApiErrorReason(error) {
    ...
    if (process.env.NODE_ENV === 'development') {
        ...
        if (error.data) {
            ...
            if (error.data.error) {
                description.error = typeof error.data.error === 'object' ? JSON.stringify(error.data.error) : error.data.error;
            }
        }
