...
import { BankAccountEditForm } from 'application/administration/donor/forms';
...
class DonorBankAccountEditViewStore extends BaseEditViewStore {
            ...
            actions: {
                get: async () => {
                    if (props.bankAccount) {
                        ...
                        const data = await rootStore.application.administration.donorStore.getBankAccount(props.editId);
                        return {
                            name: data.name,
                ...
                update: async (resource) => {
                    ...
                    await rootStore.application.administration.donorStore.updateBankAccount(resource);
                    if (this.imageUploadStore.files && this.imageUploadStore.files.length === 1) {
                        await rootStore.application.administration.donorStore.uploadDonorBankAccount(this.imageUploadStore.files[0], this.donorId, resource.id);
                    }
                ...
                create: async (resource) => {
                    ...
                    const data = await rootStore.application.administration.donorStore.createBankAccount({
                        donorId: this.donorId,
                        ...resource
                    });
                    if (this.imageUploadStore.files && this.imageUploadStore.files.length === 1) {
                        await rootStore.application.administration.donorStore.uploadDonorBankAccount(this.imageUploadStore.files[0], this.donorId, data.response);
                    }
    ...
    async getDonorInfo() {
        if (!this.donor) {
            this.donor = await this.rootStore.application.administration.donorStore.getDonor(this.donorId, {
