...
const PendingDonationListTemplate = function({ pendingDonationViewStore, t }) {
	...
	return (
		<ApplicationListLayout store={pendingDonationViewStore} authorization={authorization} loading={loaderStore.loading}>
			<Content>
				<div className="card--med card--primary container--sml u-mar--bottom--sml">
					...
					<div className="u-mar--bottom--sml">
						...
						!isNullOrWhiteSpacesOrUndefinedOrEmpty(form.$('paymentNumber').value) &&
						tableStore &&
						tableStore.selectedItems &&
						tableStore.selectedItems.length > 0 ? (
							<div>
								Check Numbers From: {form.$('paymentNumber').value} to{' '}
								{tableStore.selectedItems.length + Number(form.$('paymentNumber').value) - 1}
							</div>
						) : null}
						{!isNullOrWhiteSpacesOrUndefinedOrEmpty(form.$('paymentTypeId').value) &&
							paymentTypeDropdownStore.value &&
						...
						{!isNullOrWhiteSpacesOrUndefinedOrEmpty(form.$('paymentTypeId').value) &&
							paymentTypeDropdownStore.value &&
							paymentTypeDropdownStore.value.abrv === 'charity-account' && (
								<div>
									Next charity account transfer number:{' '}
									<span className="btn btn--sml btn--link" onClick={onAchNextPaymentNumberClick}>
										CA-{achBatchCurrentNumber + 1}
									</span>
								</div>
							)}
					</div>
			...
			<BaasicModal modalParams={donationLogModalParams}>
				<DonationLog loading={loaderStore.loading} />
			</BaasicModal>
