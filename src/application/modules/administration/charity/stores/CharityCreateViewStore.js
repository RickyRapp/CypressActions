...
import { CharityCreateForm } from 'application/administration/charity/forms';
            ...
            actions: () => {
                return {
                    create: async (resource) => {
                        ...
                        const data = await this.rootStore.application.administration.charityStore.createCharity(model);
                        if (resource.isNewBankAccount && this.imageUploadStore.files && this.imageUploadStore.files.length === 1) {
                            await this.rootStore.application.administration.charityStore.uploadBankAccount(this.imageUploadStore.files[0], data.response.charityId, data.response.bankAccountId);
                        }
                    }
                }
            },
