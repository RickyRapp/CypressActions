...
class QueryUtility {
  ...
  @action async initialize(updateUrlParams = true) {
    this.updateFilter();
  ...
  @action.bound changeOrder = (sort, updateUrlParams = true) => {
    ...
    if (
      ...
      this.filter.orderDirection === sortDirection.asc
    ) {
      direction = sortDirection.desc;
    }
    this.filter.orderBy = sort;
    this.filter.orderDirection =
      this.filter.orderBy === null ? null : direction;
  ...
  getValidQueryParams(queryParams) {
    return _.omitBy(queryParams, (param, key) => {
      ...
      return this.ignoreQueryParams
        ? _.some(this.ignoreQueryParams, ignore => ignore === key)
        : false;
    });
  }
  ...
  updateUrlParams = () => {
    ...
    if (this.onTransformQueryParams) {
      queryParams = this.onTransformQueryParams(queryParams);
    }
  ...
  updateFilter = (useDefault = false) => {
    ...
    this.filter.pageNumber =
      parseInt(page < 0 ? 1 : page) || this.filter.pageNumber;
    ...
    if (others) {
      ...
      const self = this;
      const evaluateQueryParam = (queryParamKey, queryParamValue) => {
        if (self.queryParamMap && self.queryParamMap[queryParamKey]) {
          // custom mapping function for specified properties
          return self.queryParamMap[queryParamKey](
            queryParamKey,
            queryParamValue
          );
        }
      ...
      _.assignWith(this.filter, mapParams, (objValue, srcValue, key) => {
        return _.isUndefined(srcValue)
          ? objValue
          : evaluateQueryParam(key, srcValue);
      });
    }
  };
...
function withoutProperties(obj, properties) {
  return _.omitBy(obj, (param, key) => {
    return !_.isUndefined(properties) && !_.isNull(obj)
      ? _.some(properties, ignore => ignore === key)
      : false;
  });
}
