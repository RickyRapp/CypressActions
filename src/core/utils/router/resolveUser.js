...
async function resolvePlatformUser(fromState, toState, routerStore) {
    ...
    const isAppUser = (u) => u.isApplicationUser !== undefined && u.isApplicationUser === true;
    const resolveUnauthorized = () => authStore.isAuthenticated ? Promise.reject(new RouterState('unauthorized')) : Promise.reject(new RouterState(routerState.rootState.getLoginRoute()));
...
async function resolveApplicationUser(fromState, toState, routerStore) {
    ...
    const resolveUnauthorized = () => authStore.isAuthenticated ? Promise.reject(new RouterState('unauthorized')) : Promise.reject(routerState.rootStore.getLoginRoute(baasicApplication.getApiKey()));
