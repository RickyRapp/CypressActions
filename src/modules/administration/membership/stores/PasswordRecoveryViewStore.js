...
import { PasswordRecoveryForm } from 'modules/administration/membership/forms';
...
class PasswordRecoveryViewStore extends BaseViewStore {
  ...
  @action.bound async requestReset({ email }) {
    ...
    try {
      const recoverUrl = `${
        ...
        }/password-change/{?passwordRecoveryToken}`;
      ...
      this.rootStore.notificationStore.success(
        'An e-mail containing a link to reset the password on your account has been sent.'
      );
      this.goToLogin();
    } catch (ex) {
      if (ex.data) {
  ...
  @action.bound goToLogin() {
    this.rootStore.routerStore.navigate('master.app.membership.login', {
