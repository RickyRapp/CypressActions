...
import { BaasicButton, BaasicTable, TableFilter, EmptyState, NullableSwitch, BaasicInput } from 'core/components';
...
const DonorListTemplate = function({ donorViewStore }) {
	const { tableStore, routes, queryUtility, authorization, accountTypes } = donorViewStore;
						...
						<TableFilter queryUtility={queryUtility} showDefaultSearchFilter={false}>
							...
							<div className="col col-sml-12 u-mar--top--sml u-mar--bottom--med">
								{accountTypes && (
									<NullableSwitch
										value={
											!isSome(queryUtility.filter['accountTypeId'])
												? null
												: queryUtility.filter['accountTypeId'] === _.find(accountTypes, { abrv: 'regular' }).id
										}
										onChange={newValue =>
											(queryUtility.filter['accountTypeId'] = !isSome(newValue)
												? null
												: _.find(accountTypes, { abrv: newValue ? 'regular' : 'private' }).id)
										}
