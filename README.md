...
Baasic reactjs bootstrap includes modules for easier Baasic integration. Every module has its own configuration/initialization file (`config.js` in module root). Example of module initialization:
...
(function () {
    moduleProvider.register({
        routes: [...],
        menu: [...],
        getStore: function (rootStore) {
            return { "baasic.user": new UserStore(rootStore) }
        }
    });
})();
```
