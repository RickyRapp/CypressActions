    ...
    return (
        <div className='form__group--inline'>
            <div className='flex'>
                {
                    showLabel && (
                        <div className='form__group__label form__group__label--lowercase ' htmlFor={field.id}>{t(field.label)}{requiredMark}</div>)
                }
            ...
            {renderIf(isSome(field.localizedError))(
                <p className='type--tiny type--color--error w--100'>{field.localizedError}</p>
            )}
        </div>
    )
});
