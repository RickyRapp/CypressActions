...
class BaasicDropdownStore {
    ...
    @computed get loading() { return this.loaderStore.loading; }
    @computed get value() {
    ...
    @computed get defaultValue() {
        ...
        const item = !_.isNull(options.defaultItem) && !_.isUndefined(options.defaultItem)
            ? options.defaultItem
    ...
    async defaultFilter(filterTerm) {
        return this.actions.fetchFunc ? 
            this.filterAsync(filterTerm) : this.filterInMemory(filterTerm);       
    }

    @action.bound
    filterInMemory(filterTerm) {
        const items = _.filter(this.items.slice(), i => filterTerm && i[this.options.textField] === filterTerm);
        this.setItems(items);
    }
