...
export default class MenuStore {
  ...
  @computed get secondaryMenuVisible() {
    return (
      (this.selectedPath && this.selectedPath.length > 0) || this.menuPinned
    );
  }

  @computed get secondaryMenu() {
    const path =
      this.selectedPath.length === 0 ? this.activePath : this.selectedPath;
  ...
  @computed get terniaryMenu() {
    ...
    const parent = _.find(this.secondaryMenu, item =>
      item.isActiveByPath(this.selectedPath)
    );
    return parent ? parent.subMenu : [];
  }
  ...
  @computed get tabMenu() {
    const parent = _.find(this.secondaryMenu, item =>
      item.isActiveByPath(this.activePath)
    );
    const activeSecondaryItems = parent ? parent.subMenu : [];

    const terniaryActive = _.find(activeSecondaryItems, item =>
      item.isActiveByPath(this.activePath)
    );
...
function findActiveMenuItem(menu, route) {
  ...
  for (let i = 0; i < menu.length; i++) {
    ...
    if (item.route) {
      const menuRoute =
        typeof item.route === 'function' ? item.route() : item.route;
      if (menuRoute === route) {
...
function getDifference(menuRoute, route, lastDifference = 0) {
  ...
  return _.difference(menuParts.slice(0, -1), routeParts.slice(0, -(diff + 1)))
    .length;
}
