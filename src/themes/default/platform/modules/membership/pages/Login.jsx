...
function LoginTemplate({ currentView, t }) {
    ...
    return <section className="w--300--500 align--h--center padd--top--med">
            ...
            <form className="form card card--xlrg card--primary spc--top--med" onSubmit={loginForm.onSubmit}>
                ...
                {loginForm.error && <div>
                        <p className="type--color--warning">
                            {loginForm.error}
                        </p>
                    </div>}
                <div className="clearfix spc--top--sml">
                    ...
                    <div className="pull">
                        <BaasicButton className="btn btn--med btn--primary" type="submit" disabled={loginForm.submitting} icon={loginForm.submitting || loginForm.validating ? "synchronize-arrows-1 rotate" : ""} label="Sign In"/>
                        {loading && <span className="loader--login padd--top--med">
                                <Loader />
                            </span>}
                    </div>
                </div>
