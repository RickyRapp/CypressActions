    ...
    return (
        <div>
            <div className="spc--bottom--sml">
                ...
                <a className="type--sml type--color--grey push" href="#">
                    Mark All as Read
				</a>
            </div>
            <div className="messages">
                {props.messages.length === 0 ? (
                    <React.Fragment>No messages</React.Fragment>
                ) : (
                        <React.Fragment>
                            {props.messages.map((message, idx) => (
                                <div key={idx} className={'messages__item ' + (message.isRead ? '' : 'unread')}>
                                    <BaasicMessageItem key={idx} {...message} markAsRead={props.markAsRead} />
                                </div>
                            ))}
                        </React.Fragment>
                    )}
